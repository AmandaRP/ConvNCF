...
#Compute outper product:
user_embedding_transpose <- user_embedding %>% layer_reshape(target_shape = c(K,1))
outer_product <- k_dot(user_embedding_transpose, item_embedding)

#outer_product <- layer_dot(
#  inputs = list(user_embedding_transpose, item_embedding),
#  name = "outer_product")

outer_product %>% layer_conv_2d()
